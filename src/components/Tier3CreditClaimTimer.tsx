...
import {
  tier3CreditClaimDurationMinutes,
  tier3CreditClaimStartISO,
} from "@/lib/tier3-challenge";

const START = new Date(tier3CreditClaimStartISO).getTime();
const END = START + tier3CreditClaimDurationMinutes * 60 * 1000;
...
export function Tier3CreditClaimTimer({
  compact = false,
}: {
  compact?: boolean;
}) {
  ...
  const statusLabel =
    phase === "upcoming"
      ? "Credit claim starts in"
      : phase === "live"
        ? "Tier 3 · Claim credits"
        : "Credit claim · Ended";

  const hint =
    phase === "upcoming"
      ? `${tier3CreditClaimDurationMinutes}-minute window · one email per team`
      : phase === "live"
        ? `${tier3CreditClaimDurationMinutes}-minute window · submit team name + one ImagineArt email`
        : "Credit claim closed — use the account you registered";
