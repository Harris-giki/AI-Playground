...
import {
  tier3ChallengeDurationHours,
  tier3ChallengeStartISO,
} from "@/lib/tier3-challenge";

const START = new Date(tier3ChallengeStartISO).getTime();
const END = START + tier3ChallengeDurationHours * 60 * 60 * 1000;
...
export function Tier3ChallengeTimer({ compact = false }: { compact?: boolean }) {
  ...
  const hint =
    phase === "upcoming"
      ? `${tier3ChallengeDurationHours}-hour sprint · half assets · half film`
      : phase === "live"
        ? `${tier3ChallengeDurationHours}-hour sprint · submit film + workflow before time runs out`
        : "Tier 3 submissions closed for this window";
