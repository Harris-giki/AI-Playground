...
export const metadata: Metadata = {
  title: "Tier 3 · Make the Film · AI Playground · ImagineArt",
  description:
    "Tier 3 for shortlisted teams: a 5-hour sprint to generate assets and make a 2–3 minute film. Half the time on assets, half on assembly.",
};

export default function Tier3Page() {
  const { brief, sprint, criteria, rules, submission } = tier3Challenge;
    ...
    <>
        ...
        <div className="relative z-10 max-w-[1200px] mx-auto">
          <ScrollReveal>
            <div className="section-head mb-6">
              ...
              <span className="label-eyebrow">{tier3Challenge.heroEyebrow}</span>
            </div>
            <h1 className="title-hero mb-5 descender-safe max-w-[900px]">
              {tier3Challenge.title}
            </h1>
            <p className="prose-body mb-8 max-w-[620px]">
              {tier3Challenge.subtitle}
            </p>
          ...
          <ScrollReveal delay={0.05}>
            <article className="challenge-prompt tier3-prompt" data-hover="READ">
              <p className="tier3-prompt__direction descender-safe">
                &ldquo;{tier3Challenge.direction}&rdquo;
              </p>
      ...
      <section data-scroll-section className="site-section px-6 sm:px-12 py-28">
        <div className="max-w-[1200px] mx-auto">
          <ScrollReveal>
            <div className="section-head mb-6">
              <span className="label-eyebrow">{sprint.label}</span>
            </div>
            <h2 className="title-display mb-10 descender-safe max-w-[720px]">
              {sprint.title}
            </h2>
          ...
          <ScrollReveal delay={0.05}>
            <article className="challenge-prompt" data-hover="READ">
              <p className="label-day mb-4">{sprint.heading}</p>
              <div className="challenge-prompt__body">
                {sprint.paragraphs.map((paragraph, index) => (
                  <p key={index}>{paragraph}</p>
                ))}
              </div>
            </article>
          ...
          <ScrollReveal delay={0.08}>
            <div className="tier3-logistics mt-8">
              {sprint.meta.map((item) => (
                <article
                  key={item.label}
                  className="tier3-logistics__card"
                  data-hover={item.label.toUpperCase()}
                >
                  <p className="label-eyebrow mb-3">{item.label}</p>
                  <p className="title-space descender-safe">{item.value}</p>
                </article>
              ))}
            </div>
          </ScrollReveal>
        ...
        <div className="max-w-[1200px] mx-auto">
          ...
          <ScrollReveal delay={0.15}>
            <div className="challenge-cta-row mt-14 flex flex-wrap gap-4">
              ...
              <Link href="/tier-2" data-hover="TIER 2" className="btn-ghost">
                Tier 2 brief
              </Link>
      ...
      >
        <div className="max-w-[1200px] mx-auto">
          <ScrollReveal>
            ...
            <p className="prose-body mb-8 max-w-[560px]">{submission.intro}</p>
